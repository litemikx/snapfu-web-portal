...
async function updateConnectionRecord(updateRecord) {

    var token = JSON.parse(sessionStorage.getItem('token'))['token'];

    return fetch(api_host + '/api/connections/update/' + updateRecord._id, {
        method: 'POST',
        headers: {
...
export default function UpdateRecord({ updateRecord }) {
    console.log('update:' + JSON.stringify(updateRecord));
    ...
    const handleSubmit = async e => {
        ...
        var res = await updateConnectionRecord({
        ...
        return (
            <div className="updaterecord-form-wrapper">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div>
                        <button disabled={!validFilePath ? true : false}>Update</button>
                    </div>
                </form>
                <p>
                    {responseMessage}
